    ...
    public class MapperProfile : Profile
    {
        ...
        { 
            ...
            CreateMap<Shared.Model.MovieModel, Data.Movie>()
                .ForMember(x => x.Id, map => map.Ignore())
                .ForMember(x => x.Categories, map =>
                map.MapFrom((model, entity) =>
                {
                    var current = entity.Categories.Select(x =>
                    x.Category).ToArray();
                    var categories = entity.Categories.ToList();
                    categories.AddRange(model.Categories.Where(x =>
                    !current.Contains(x)).Select(x => new
                    Data.MovieCategory
                    {
                        Category = x
                    }));
                ...
                .ForMember(x => x.Actors, map => map.MapFrom((model, entity) =>
                {
                    ...
                    actors.AddRange(model.ActorIds.Where(x =>
                    !current.Contains(x)).Select(x => new Data.MovieActor
                    {
                        PersonId = x
                    }));
